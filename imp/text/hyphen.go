...
func (h *Hyphenator) addPattern(pattern string) {
	...
	for i := 0; i < len(p); i++ {
		if p[i] >= '0' && p[i] <= '9' {
			points = append(points, int(p[i]-'0'))
		} else {
	...
	h.entries = append(h.entries, hEntry{string(chars), points})
}
...
func (h *Hyphenator) Hyphenate(word string) []string {
	...
	points := make([]int, len(search))
	for i := range search {
		for j := range search[i:] {
			entry := h.entries.Find(search[i : i+j])
			if entry == nil {
				continue
			}
			for k, p := range entry.Points {
				if p > points[i+k] {
					points[i+k] = p
				}
			}
...
func (p hEntries) Len() int {
	return len(p)
}
func (p hEntries) Less(i, j int) bool {
	return p[i].Word < p[j].Word
}
func (p hEntries) Swap(i, j int) {
	p[i], p[j] = p[j], p[i]
}

func (p hEntries) Find(word string) *hEntry {
	pos := sort.Search(len(p), func(i int) bool {
		return p[i].Word >= word
	})
	if pos >= len(p) || p[pos].Word != word {
		return nil
	}
	return &p[pos]
}

var defaultHyphenator = NewHyphenator(strings.Fields(patterns))

func Hyphenate(word string) []string {
	return defaultHyphenator.Hyphenate(word)
}
